...
class AstRuntime {
  ...
  Future callMethod(String methodName, {List? params}) async {
    if (_astClass != null) {
      return _astClass!.callMethod(methodName, params ?? []);
    }
  }
...
class AstClass {
  ...
  factory AstClass.fromAst(Map ast, {AstVariableStack? variableStack}) {
    if (ast['type'] != astNodeNameValue(AstNodeName.classDeclaration)) {
      throw ArgumentError(
          'AstClass.fromAst: type is not AstNodeName.classDeclaration');
    }
...
class AstFunction {
  ...
  String name;
  bool isAsync;
  AstFunction(this.name, this._parameterList, this._body, this.isAsync);
  ...
  factory AstFunction.fromAst(Map ast) {
    ...
    throw ArgumentError(
        'AstFunction.fromAst: type is not AstNodeName.methodDeclaration or AstNodeName.functionDeclaration');
  }
}
...
Future _executeBodyExpression(
  ...
  if (expression.isVariableDeclarationList) {
    ...
    var params = [];
    var function = expression.asFunctionDeclaration;
    for (var p in function.expression.parameterList) {
    ...
    for (var p in function.parameterList) {
      params.add(variableStack.getVariableValue(p.name)?.value);
    }
    return AstFunction.fromExpression(expression)
        .invoke(params, variableStack: variableStack);
...
List _executeListLiteral(
  ...
  var list = [];
  for (var e in listLiteral.elements) {
...
Future _executeForStatement(
  ...
  if (forStatement.type == ForLoopParts.forEachPartsWithDeclaration) {
    ...
    while (condition) {
      //解析循环步长
      if (forLoopParts.updater?.isPrefixExpression ?? false) {
        _executePrefixExpression(
            forLoopParts.updater!.asPrefixExpression, variableStack);
      } else if (forLoopParts.updater?.isAssignmentExpression ?? false) {
        await _executeAssignmentExpression(
...
dynamic _executeIndexExpression(
  ...
  if (target != null) {
    if (target is List) {
      var index = indexExpression.index.asNumericLiteral.value.toInt();
