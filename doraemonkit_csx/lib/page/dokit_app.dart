...
class DoKitApp extends StatefulWidget implements IDoKitApp {
  ...
  @override
  State<StatefulWidget> createState() => _DoKitAppState();
}
...
class _DoKitWrapper extends StatelessWidget {
  ...
  void ensureDokitObserver(List<NavigatorObserver> navigatorObservers) {
    if (!navigatorObservers.any((element) => element is CsxKitNavObserver)) {
      navigatorObservers.add(CsxKitNavObserver());
    }
  }
}
          ...
          _MediaQueryFromWindow(
            Localizations(
              ...
              child: ScaffoldMessenger(
                child: Overlay(key: doKitOverlayKey),
              ),
            ),
          ),
        ],
...
class _MediaQueryFromWindow extends StatefulWidget {
  const _MediaQueryFromWindow(this.child);
...
OverlayEntry enterNewOverLayer(Widget Function(OverlayEntry?) builder) {
  OverlayEntry? entry;
  entry = OverlayEntry(
    builder: (cont) => GestureDetector(
      ...
      child: ColoredBox(
        ...
        child: builder.call(entry),
      ),
    ),
  );
  doKitOverlayKey.currentState?.insert(entry);
  return entry;
}
