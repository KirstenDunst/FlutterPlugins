...
class HttpRequest {
  ...
  void add(String parameter) {
    ...
    var kit = ApmKitManager.instance.getKit(ApmKitName.kitHttp) as HttpKit?;
    kit?.listener?.call();
  }
}
                ...
                text: TextSpan(
                  children: <InlineSpan>[
                    ...
                    TextSpan(
                      text:
                          '  ${widget.item.method}'
                          '  Cost:${widget.item.response.endTimestamp > 0 ? ('${widget.item.response.endTimestamp - widget.item.startTimestamp}ms') : '-'} '
