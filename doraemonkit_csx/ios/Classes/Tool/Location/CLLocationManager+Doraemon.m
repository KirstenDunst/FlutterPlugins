...
- (void)doraemon_swizzleLocationDelegate:(id)delegate {
    if (delegate) {
        [self doraemon_swizzleLocationDelegate:[GPSMockerTool shareInstance]];
        [[GPSMockerTool shareInstance] addLocationBinder:self delegate:delegate];
        ...
        for(unsigned i = 0; i < count; i++)
        {
            ...
            if ([delegate respondsToSelector:sel]) {
                if (![[GPSMockerTool shareInstance] respondsToSelector:sel]) {
